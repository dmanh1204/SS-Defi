        ...
        >
            <div className="flex flex-col gap-10 px-[31px] py-20 md:mx-[60px] min-[1440px]:mx-[98px] min-[1440px]:flex-row min-[1440px]:justify-between">
                ...
                <div className="hidden flex-col gap-6 min-[1440px]:flex">
                    <div className="font-['Exo_2'] text-base font-bold text-white min-[1440px]:text-xl">Community</div>
                    <SocialCard />
                </div>
            </div>
        </div>
...
const MoreAbout = () => {
    const subItemList = ['About us', 'Contact us', 'FAQ', 'Brand Kit', 'Documents'];
...
const Products = () => {
    const subItemList = ['KYC Project', 'Public/Private Sale', 'Airdrop', 'Locking'];

    return (
        <div className="inline-flex flex-col items-start gap-[18px] md:gap-4">
            <div className="font-['Exo_2'] text-base font-bold text-white min-[1440px]:text-xl">Products</div>
            <div className="flex flex-col items-start gap-2">
                {subItemList.map((item, idx) => (
                    <div key={idx} className="font-['Inter'] text-sm font-normal text-[#C6C6C6] min-[1440px]:text-base">
                        {item}
                    </div>
                ))}
            </div>
...
const FooterHeading = (props) => {
    return (
        <div className="flex flex-col items-center justify-center gap-3 md:items-start">
            <img src={icons.offical_logo} alt="logo" className="h-12" onClick={() => props.navigation(route.home)} />
            <div className="font-['Inter'] text-xs font-medium text-[#C6C6C6]">© 2023 STARKSPORT</div>
        </div>
    );
};
